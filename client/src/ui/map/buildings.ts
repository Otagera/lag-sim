...
import { getLGAGeometry, lgaToZone, pointInPolygon } from './geoProjection'
...
export function generateBuildings(): Building[] {
  ...
  outer: for (const lga of lgas) {
    ...
    const aLo = Math.floor(aMin * 2),
      aHi = Math.ceil(aMax * 2)
    const bLo = Math.floor(bMin * 2),
      bHi = Math.ceil(bMax * 2)
