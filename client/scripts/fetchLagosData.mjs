...
function centroidInLagos(feature) {
  ...
  return (
    lon >= LAGOS_BBOX.minLon &&
    lon <= LAGOS_BBOX.maxLon &&
    lat >= LAGOS_BBOX.minLat &&
    lat <= LAGOS_BBOX.maxLat
  )
}

// ── Step 1: LGA boundaries from geoBoundaries ─────────────────────────────
console.log('Fetching geoBoundaries NGA ADM2 metadata...')
const meta = await fetch('https://www.geoboundaries.org/api/current/gbOpen/NGA/ADM2/').then((r) =>
  r.json(),
)
...
const lagosLGAs = {
  ...allLGAs,
  features: allLGAs.features.filter(
    (f) => LAGOS_LGA_NAMES.has(f.properties.shapeName) && centroidInLagos(f),
  ),
}
...
console.log(
  `✓ LGAs: ${lagosLGAs.features.length} features written to public/data/lagos-lgas.geojson`,
)
...
try {
  ...
  console.log(
    `✓ LCDAs: ${lcdaGeoJSON.features.length} features written to public/data/lagos-lcdas.geojson`,
  )
} catch (err) {
  console.warn(
    `  LCDA fetch failed (${err.message}) — writing empty file, map will still work without LCDA lines`,
  )
}
