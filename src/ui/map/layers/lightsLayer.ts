...
import type { MapState, MapLens, ZoneMapState } from '../../../state/mapSelectors'
...
function litFrac(infrastructure: number): number {
  ...
  return clamp(Math.pow(norm, 2.2), 0.01, 0.97)
}
...
const TRUST_STOPS: Stop[] = [
  { at: 0.00, r: 235, g:  70, b:  55 },   // furious red
  { at: 0.30, r: 240, g: 120, b:  70 },   // angry orange
  { at: 0.55, r: 245, g: 175, b: 100 },   // uneasy amber
  { at: 0.80, r: 255, g: 210, b: 140 },   // settled gold
  { at: 1.00, r: 255, g: 235, b: 180 },   // content warm white-gold
]
...
const INFRA_STOPS: Stop[] = [
  { at: 0.00, r: 230, g:  55, b:  45 },   // critical
  { at: 0.30, r: 240, g: 130, b:  55 },   // poor
  { at: 0.55, r: 245, g: 200, b:  75 },   // adequate
  { at: 0.80, r: 145, g: 220, b: 100 },   // good
  { at: 1.00, r:  90, g: 240, b: 120 },   // excellent
]

// Security lens: dangerous red → safe cool-white
const SECURITY_STOPS: Stop[] = [
  { at: 0.00, r: 225, g:  45, b:  45 },   // dangerous
  { at: 0.35, r: 238, g: 130, b:  65 },   // unsafe
  { at: 0.60, r: 238, g: 205, b: 125 },   // moderate
  { at: 0.80, r: 175, g: 218, b: 255 },   // safe
  { at: 1.00, r: 200, g: 232, b: 255 },   // very safe
]

// Youth tension lens: calm teal → burning red (HIGH tension = hot)
const YOUTH_STOPS: Stop[] = [
  { at: 0.00, r: 150, g: 228, b: 195 },   // calm
  { at: 0.30, r: 245, g: 218, b: 118 },   // mild
  { at: 0.60, r: 255, g: 158, b:  55 },   // rising
  { at: 0.80, r: 248, g:  88, b:  58 },   // hot
  { at: 1.00, r: 238, g:  48, b:  48 },   // critical
]

function getColorForLens(z: ZoneMapState, lens: MapLens): number {
  switch (lens) {
    case 'infrastructure': return interpolateStops(INFRA_STOPS,    z.infrastructure / 100)
    case 'security':       return interpolateStops(SECURITY_STOPS, z.security / 100)
    case 'youth':          return interpolateStops(YOUTH_STOPS,    z.youthTension / 100)
    default:               return trustToColor(z.trust)
  }
}
...
function windowAlpha(
  ...
  if (stability > 0.65 && crisis !== 'crisis') {
    ...
    const f =   Math.sin(t * (3 + intensity * 7) + phase)
              * Math.sin(t * (1.7 + intensity * 3) + phase * 1.4)
...
function faceDarknessAlpha(infrastructure: number, zoneType: ZoneType): number {
  ...
  const brightness = (0.18 + 0.30 * Math.pow(infrastructure / 100, 1.5)) * lerp(1.0, 0.8, heightFactor)
  ...
  if (vi) {
    ...
    if (baseAlpha >= 0.01) {
      ...
      for (let i = 0; i < 16; i++) {
        ...
        if (a < 0.005) continue
        g.moveTo(xC - lineW / 2, yC).lineTo(xC + lineW / 2, yC)
      ...
      for (let j = 0; j < 10; j++) {
        ...
        if (a < 0.005) continue
        g.moveTo(x, yS).lineTo(x, yS + pilH)
  ...
  if (island) {
    ...
    if (baseAlpha >= 0.01) {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const xC    = ox + 4 + shimX
        const yC    = oy + 192 + i * 4
        const lineW = 28 + Math.sin(t * 0.48 + i * 0.60) * 5
...
export function createLightsLayer(): MapLayer {
  ...
  function buildGlow(state: MapState) {
    ...
    for (let zi = 0; zi < CITY_ZONES.length; zi++) {
      ...
      const gIntensity = Math.pow(zd.litFraction, 1.3) * (trust / 100) * 0.5 * heightDamp
  ...
  function buildDarkness(state: MapState) {
    ...
    for (let zi = 0; zi < CITY_ZONES.length; zi++) {
      ...
      darknessG.poly(corners.flatMap(p => [p.x, p.y]))
