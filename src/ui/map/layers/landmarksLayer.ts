...
const BRIDGE_WAYPOINTS: [number, number][] = [
  [20, 34], [26, 34], [32, 34], [38, 33.5], [44, 33], [50, 33],
]
...
const VI_TOWERS: [number, number][] = [
  [59, 47], [60, 47.5], [61, 47], [60, 48.5], [59.5, 46.5],
]
...
export function createLandmarksLayer(): MapLayer {
  ...
  function drawStatic(ox: number, oy: number) {
    ...
    for (let i = 0; i < dPts.length - 1; i++) {
      ...
      if (len < 0.1) continue
      const nx = -dy / len * 3, ny = dx / len * 3  // perpendicular, 3px half-width
      g.poly([
      ...
      g.poly([
        p0.x + nx * 0.5, p0.y + ny * 0.5, p1.x + nx * 0.5, p1.y + ny * 0.5,
        p1.x - nx * 0.5, p1.y - ny * 0.5, p0.x - nx * 0.5, p0.y - ny * 0.5,
      ]).fill({ color: DECK_GLOW, alpha: 0.20 })
    }
    ...
    const apexX = (ax + bx) / 2, apexY = (ay + by) / 2 - 30
    const legSpread = 12
    // Left leg
    g.moveTo(apexX - 1, apexY).lineTo(ax - legSpread, ay).stroke({ color: PYLON_COLOR, width: 2, alpha: 0.55 })
    // Right leg
    g.moveTo(apexX + 1, apexY).lineTo(bx + legSpread, by).stroke({ color: PYLON_COLOR, width: 2, alpha: 0.55 })
    // Cross beam at mid-height
    const midY = (apexY + ay) / 2
    g.moveTo(ax - legSpread * 0.5, midY).lineTo(bx + legSpread * 0.5, midY).stroke({ color: PYLON_COLOR, width: 1.5, alpha: 0.40 })
    // Cable lines radiating from apex to deck
    const cableCount = 5
    for (let i = 1; i <= cableCount; i++) {
      const t = i / (cableCount + 1)
      const cx = ax + (bx - ax) * t, cy = ay + (by - ay) * t
      g.moveTo(apexX, apexY).lineTo(cx, cy).stroke({ color: 0x4a6a8a, width: 0.8, alpha: 0.20 })
    }

    // ── Apapa port cranes (gantry silhouettes) ────────────────────────────
    for (const [ca, cb] of APAPA_CRANES) {
      const { x, y } = isoToScreen(ca, cb, ox, oy)
      const craneH = 16, craneW = 6
      // Vertical post
      ...
      g.moveTo(x - craneW, y - craneH).lineTo(x - 1, y - craneH + 6).stroke({ color: CRANE_COLOR, width: 0.8, alpha: 0.25 })
      g.moveTo(x + craneW, y - craneH).lineTo(x + 1, y - craneH + 6).stroke({ color: CRANE_COLOR, width: 0.8, alpha: 0.25 })
    }
    ...
    const domeW = 12, domeH = 14
    g.moveTo(ntx - domeW, nty).lineTo(ntx - domeW * 0.8, nty - domeH * 0.5)
      .lineTo(ntx - domeW * 0.4, nty - domeH * 0.85)
      .lineTo(ntx, nty - domeH)
      .lineTo(ntx + domeW * 0.4, nty - domeH * 0.85)
      .lineTo(ntx + domeW * 0.8, nty - domeH * 0.5)
      .lineTo(ntx + domeW, nty)
      .stroke({ color: DOME_COLOR, width: 1.5, alpha: 0.40 })
    // Fill the dome shape
    ...
    for (const [ta, tb] of VI_TOWERS) {
      ...
      // Tower body
      g.rect(x - tw / 2, y - th, tw, th).fill({ color: TOWER_COLOR, alpha: 0.50 })
      // Antenna/spire on top
      g.moveTo(x, y - th).lineTo(x, y - th - 4).stroke({ color: TOWER_COLOR, width: 1, alpha: 0.30 })
