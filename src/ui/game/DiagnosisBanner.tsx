...
function deriveDiagnosis(s: ReturnType<typeof useGameStore.getState>): string | null {
  ...
  if (s.stats.politicalCapital < 15)                 return `Political capital critically low (${Math.round(s.stats.politicalCapital)}). Mend fences with the godfathers, deliver a popular win, or side with a faction to rebuild it.`
  ...
  if (s.stats.politicalCapital < 25)                 return `Low political capital (${Math.round(s.stats.politicalCapital)}). Court the godfathers or deliver a visible win to rebuild it.`
