...
export function WelcomeScreen({ onNewGame, onContinue, canContinue }: Props) {
  return (
    <div className="h-screen flex flex-col" style={{ backgroundColor: 'var(--background)', color: 'var(--text)' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-lg text-center">
          ...
          <h1 className="font-display font-semibold mt-2" style={{ fontSize: '30px', color: 'var(--text)' }}>
            Lagos Governor Sim
          </h1>
          <p className="mt-3 max-w-md mx-auto leading-relaxed" style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
          ...
          <div className="mt-10">
            <div className="grid grid-cols-2 gap-3 text-left max-w-md mx-auto">
              {STATS.map(({ Icon, label, desc, color }) => (
                <Surface key={label} elevation="raised" padding="12px" style={{ display: 'flex', gap: '10px' }}>
                  <span className="shrink-0 w-5 flex items-start justify-center mt-0.5">
                    <Icon className="w-4 h-4" strokeWidth={1.5} style={{ color }} />
                  </span>
                  <div>
                    <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text)' }}>{label}</p>
                    <p style={{ fontSize: '10px', marginTop: '2px', lineHeight: 1.3, color: 'var(--text-secondary)' }}>{desc}</p>
                  </div>
                </Surface>
              ))}
            </div>
