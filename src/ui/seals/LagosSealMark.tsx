...
export function LagosSealMark({
  ...
  const commonProps = { size, opacity, monogram, color }
  ...
  if (size < 64) return <CompactSealMark {...commonProps} />

  return <FullSealMark {...commonProps} />
}

type SealRenderProps = {
  size: number
  opacity: number
  monogram: string
  color: string
...
const COMPACT_RAYS = Array.from({ length: 5 }, (_, i) => i)
const FULL_RAYS = Array.from({ length: 9 }, (_, i) => i)
const BRIDGE_HANGERS = [68, 79, 90, 100, 110, 121, 132]
const FROND_STEPS = [0.12, 0.28, 0.44, 0.6, 0.76, 0.9]

function CompactSealMark({ size, opacity, monogram, color }: SealRenderProps) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      <g opacity={opacity} stroke={color} strokeWidth="7">
        ...
        <SealMonogram monogram={monogram} color={color} />
      </g>
    </svg>
  )
}

function CompactSunRays() {
  return COMPACT_RAYS.map((i) => {
...
function SealMonogram({ monogram, color }: { monogram: string; color: string }) {
  return (
    <text
      x="100"
      y="162"
...
function FullSealMark({ size, opacity, color }: SealRenderProps) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      <g opacity={opacity} stroke={color}>
        <SealRings />
        <SealArcText color={color} />
        <SealEmblem color={color} />
      </g>
    </svg>
  )
}
...
function SealArcText({ color }: { color: string }) {
  return (
    <>
...
function SealEmblem({ color }: { color: string }) {
  return (
    <>
      <path
...
function SealSun({ color }: { color: string }) {
  return (
    <>
      ...
      {FULL_RAYS.map((i) => {
        ...
        return (
          <line
            ...
            y1={82 + Math.sin(a) * 13}
            x2={100 + Math.cos(a) * (i % 2 === 0 ? 19 : 16)}
            y2={82 + Math.sin(a) * (i % 2 === 0 ? 19 : 16)}
            strokeWidth="1.4"
            strokeLinecap="round"
          />
        )
...
function PalmFronds() {
  return (
    <g strokeWidth="1.3" strokeLinecap="round">
      ...
      {FROND_STEPS.map((t) => (
        <FrondLeaf key={`lf-${t}`} t={t} side="left" />
      ))}
      <path d="M 148 148 Q 158 122 148 96" fill="none" />
      {FROND_STEPS.map((t) => (
        <FrondLeaf key={`rf-${t}`} t={t} side="right" />
      ))}
      <path
...
function FrondLeaf({ t, side }: { t: number; side: 'left' | 'right' }) {
  const y = 148 - t * 52
  const direction = side === 'left' ? -1 : 1
  const x = side === 'left' ? 52 - Math.sin(t * Math.PI) * 10 : 148 + Math.sin(t * Math.PI) * 10
  return (
    <path
      d={`M ${x} ${y} q ${direction * 9} ${-3 - t * 3} ${direction * 13} ${2 - t * 6}`}
      fill="none"
      opacity="0.85"
    />
  )
