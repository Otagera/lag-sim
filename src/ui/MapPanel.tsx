...
export function MapPanel() {
  ...
  }, [youthTension, securityIndex, infraScore, constituencyApproval])
      ...
      >
        <button
          ...
          style={{
            ...
            borderBottom:
              layer === null ? '2px solid var(--accent-solid)' : '2px solid transparent',
          }}
        >
          Approval
        </button>
        {layers.map(([key, cfg]) => (
          <button
            ...
            style={{
              ...
              borderBottom:
                layer === key ? '2px solid var(--accent-solid)' : '2px solid transparent',
            }}
          >
            {cfg.label}
          </button>
      ...
      <div style={{ position: 'relative', width: '100%', height: 340, overflow: 'hidden' }}>
        ...
        {mapMode === 'night' &&
          (() => {
            ...
            const nightLens: MapLens = layer === 'youthTension' ? 'youth' : (layer ?? 'approval')
            return (
                ...
                <div style={{ position: 'absolute', top: 10, left: 48, zIndex: 10 }}>
                  ...
                  <span
                    style={{
                      ...
                      fontSize: 10,
                      fontWeight: 600,
                      backgroundColor: STATUS_FILL[st],
                      color: '#fff',
                    }}
                  >
                    ...
                    >
                      <div
                        style={{
                          ...
                          background:
                            p.status === 'stalled' ? 'var(--error-9)' : 'var(--accent-solid)',
                          transition: 'width 0.3s ease',
                        }}
                      />
                    </div>
                    <p style={{ fontSize: 10, color: 'var(--text-secondary)', margin: '3px 0 0' }}>
                      {p.effectiveProgress.toFixed(0)}% complete · ₦{p.weeklyDraw.toFixed(2)}bn/wk
                      draw
                    </p>
                  </div>
                ))}
