import { BASE_W, BASE_H, DISTRICTS } from '../ui/mapData'
...
function generateInZone(rng: () => number, z: BuildZone, roofs: string[], walls: string[], shadows: string[], accent: string | undefined): ResolvedBuilding[] {
  ...
  for (let x = z.cx - z.rx; x < z.cx + z.rx; x += z.gridW) {
    for (let y = z.cy - z.ry; y < z.cy + z.ry; y += z.gridH) {
      ...
      const px = x + rngRange(rng, -z.gridW * 0.3, z.gridW * 0.3)
      const py = y + rngRange(rng, -z.gridH * 0.3, z.gridH * 0.3)
...
function generateTrees(rng: () => number, allBuildings: ResolvedBuilding[], zones: BuildZone[], extraCount: number): ResolvedTree[] {
  ...
  for (let i = 0; i < extraCount; i++) {
    for (let attempt = 0; attempt < 20; attempt++) {
      const px = rngRange(rng, 10, BASE_W - 10)
      const py = rngRange(rng, 10, BASE_H - 10)
      if (avoidTree(px, py)) continue
      trees.push({
        cx: px, cy: py,
...
function generateRoadHierarchy(rng: () => number, zones: BuildZone[]): ResolvedRoad[] {
  ...
  roads.push({ points: arterialPts, w: 14, level: 'primary', sortY: 95 })
  ...
  roads.push({ points: ringPoints, w: 10, level: 'primary', sortY: 95 })
  ...
  for (const [k1, k2, cx, cy] of primary) {
    ...
    roads.push({ points: pts, w: 10, level: 'primary', sortY: (z1.cy + z2.cy) / 2 })
  }
  ...
  for (const [k1, k2] of secondary) {
    ...
    if (k2 === 'ringway') {
      // Connect to nearest ring road point
  ...
  // Local roads: grid within each zone (denser now with larger zones)
  for (const z of zones) {
...
function generateBoats(rng: () => number): ResolvedBoat[] {
  ...
  for (const [cx, cy, type] of spots) {
    ...
    boats.push({
      ...
      type: type as 'ferry' | 'canoe' | 'cargo',
...
export function generateCityScene(seed: number): ResolvedScene {
  ...
  const zones: BuildZone[] = [
    ...
    { districtKey: 'alimosho', cx: 90, cy: 100, rx: 88, ry: 65, type: 'residential', density: 0.88, gridW: 6, gridH: 5, minW: 5, maxW: 9, minD: 3, maxD: 6, minH: 3, maxH: 8, depthLayer: 0 },
    // Mainland: center (Oshodi — mixed-use, higher)
    { districtKey: 'oshodi', cx: 235, cy: 105, rx: 82, ry: 65, type: 'mixedUse', density: 0.85, gridW: 6, gridH: 5, minW: 5, maxW: 10, minD: 4, maxD: 7, minH: 5, maxH: 14, depthLayer: 1 },
    // Mainland: east (Surulere — mixed-use)
    { districtKey: 'surulere', cx: 370, cy: 100, rx: 82, ry: 60, type: 'mixedUse', density: 0.82, gridW: 6, gridH: 5, minW: 5, maxW: 10, minD: 4, maxD: 7, minH: 5, maxH: 14, depthLayer: 0 },
    // Fill gap between Oshodi and Surulere
    { districtKey: 'surulere', cx: 305, cy: 105, rx: 25, ry: 55, type: 'mixedUse', density: 0.75, gridW: 7, gridH: 6, minW: 4, maxW: 8, minD: 3, maxD: 6, minH: 4, maxH: 10, depthLayer: 0 },
    ...
    // Island: Lagos Island
    { districtKey: 'lagosIsland', cx: 135, cy: 218, rx: 48, ry: 32, type: 'mixedUse', density: 0.82, gridW: 5, gridH: 4, minW: 5, maxW: 9, minD: 4, maxD: 6, minH: 5, maxH: 12, depthLayer: 1 },
    // Island: Victoria Island (high-density business district)
    { districtKey: 'victoriaIsland', cx: 310, cy: 212, rx: 52, ry: 32, type: 'glassTower', density: 0.80, gridW: 7, gridH: 5, minW: 7, maxW: 12, minD: 4, maxD: 7, minH: 14, maxH: 30, depthLayer: 1 },
    ...
    // South: Lekki
    { districtKey: 'lekki', cx: 230, cy: 298, rx: 85, ry: 32, type: 'mixedUse', density: 0.78, gridW: 9, gridH: 7, minW: 5, maxW: 10, minD: 4, maxD: 7, minH: 5, maxH: 14, depthLayer: 2 },
  ]

  // Dense cores (small tight zones for visual contrast)
  zones.push({
    districtKey: 'alimosho', cx: 85, cy: 100, rx: 28, ry: 22, type: 'residential', density: 0.92, gridW: 5, gridH: 4, minW: 4, maxW: 7, minD: 3, maxD: 6, minH: 5, maxH: 12, depthLayer: 0,
  })
  zones.push({
    districtKey: 'oshodi', cx: 235, cy: 105, rx: 20, ry: 18, type: 'mixedUse', density: 0.90, gridW: 6, gridH: 5, minW: 5, maxW: 9, minD: 4, maxD: 7, minH: 8, maxH: 18, depthLayer: 1,
  })
  zones.push({
    districtKey: 'surulere', cx: 370, cy: 100, rx: 18, ry: 16, type: 'mixedUse', density: 0.88, gridW: 6, gridH: 5, minW: 5, maxW: 9, minD: 4, maxD: 7, minH: 8, maxH: 16, depthLayer: 0,
  })
  zones.push({
    districtKey: 'victoriaIsland', cx: 310, cy: 210, rx: 18, ry: 14, type: 'glassTower', density: 0.90, gridW: 14, gridH: 10, minW: 8, maxW: 14, minD: 5, maxD: 8, minH: 24, maxH: 40, depthLayer: 1,
  })
  ...
  // Second market: Oshodi/Tejuosho (mainland center)
  ...
  const pins: ResolvedPin[] = DISTRICTS.map((d, i) => ({
    cx: d.centroid[0],
    cy: d.centroid[1] - 18,
    color: pinColors[i % pinColors.length],
    sortY: d.centroid[1] + 8,
  }))
  ...
  // ---- Organic landmasses ----
  // Mainland (large connected mass covering periphery, alimosho, oshodi, surulere)
  ...
  // Makoko micro-islands (spread further apart for visible water channels)
