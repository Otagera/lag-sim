...
export const ALL_GOALS: Goal[] = [
...

  {
    ...
    description: 'Fiscal sovereignty is the foundation of every other ambition. This path means building IGR until the state can stand without FAAC crutches, eliminating the deficit, and building a cash reserve that insulates the city from Abuja\'s mood swings. Hard bureaucracy. Quiet transformation.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => s.stats.cashReserve < 5 ? `Cash reserve is ₦${s.stats.cashReserve.toFixed(1)}bn. Need ₦5bn for a real cushion.` : null,
      },
      {
        label: 'Eliminate the deficit — IGR must cover expenditure',
        progress: (s) => s.stats.igr >= s.stats.expenditure ? 1 : Math.min(1, s.stats.igr / s.stats.expenditure),
        blockingText: (s) => s.stats.igr < s.stats.expenditure ? `Spending ₦${s.stats.expenditure.toFixed(1)}bn/wk but earning ₦${s.stats.igr.toFixed(1)}bn. Close the gap.` : null,
      },
      {
        label: 'Sustainable debt — debt stock below annual IGR',
        progress: (s) => s.stats.debtStock <= 0 ? 1 : Math.min(1, (s.stats.igr * 52) / s.stats.debtStock),
        blockingText: (s) => s.stats.debtStock > s.stats.igr * 52 ? `Debt (₦${s.stats.debtStock.toFixed(0)}bn) exceeds annual IGR.` : null,
      },
    ],
...

  {
    ...
    description: 'The Safe City path is about the slow, grinding work of public safety: forensic capability that closes cases, a visible security presence that deters, and a justice system the public actually trusts. It pairs naturally with the forensics research domain. Every point of securityIndex is a life less interrupted.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => s.stats.securityIndex < 80 ? `Security is at ${s.stats.securityIndex.toFixed(0)}. Needs to reach 80 for real safety.` : null,
      },
      {
        label: 'Youth tension below 30%',
        progress: (s) => Math.max(0, Math.min(1, (60 - s.stats.youthTension) / 30)),
        blockingText: (s) => s.stats.youthTension > 30 ? `Youth tension is ${s.stats.youthTension.toFixed(0)} — above the 30% threshold for calm.` : null,
      },
      {
        label: 'Public trust at 75% or above',
        progress: (s) => Math.min(1, s.stats.publicTrust / 75),
        blockingText: (s) => s.stats.publicTrust < 75 ? `Only ${s.stats.publicTrust.toFixed(0)}% of Lagosians trust the government. Need 75%.` : null,
      },
    ],
...

  {
    ...
    description: 'Nollywood, Afrobeats, Lagos Fashion Week — the creative economy already runs the cultural conversation. This path is about formalising it: tax credits for productions, 24-hour power in entertainment districts, and a regulatory environment that lets the industry scale. The creativeEconomy secondary faction measures this sector\'s health.',
    flavorClosing: 'Lagos culture is no longer just exported — it is funded, formalised, and flourishing at home.',
    targets: [
      {
        label: 'Creative economy faction at 70+',
        progress: (s) => Math.min(1, (s.secondaryFactions.creativeEconomy ?? 45) / 70),
        blockingText: (s) => (s.secondaryFactions.creativeEconomy ?? 45) < 70 ? `Creative economy is at ${(s.secondaryFactions.creativeEconomy ?? 45).toFixed(0)}. Needs 70.` : null,
      },
      {
        ...
        blockingText: (s) => `Surulere (${(s.constituencyApproval.surulere ?? 0).toFixed(0)}) / Lagos Island (${(s.constituencyApproval.lagosIsland ?? 0).toFixed(0)}) — both need 70+.`,
      },
      {
        label: 'IGR contribution from creative sector',
        progress: (s) => Math.min(1, s.stats.igr / 15),
        blockingText: (s) => s.stats.igr < 15 ? `Weekly IGR of ₦${s.stats.igr.toFixed(1)}bn is below the ₦15bn creative-economy threshold.` : null,
      },
    ],
    relevantFactions: ['civilSocietyMedia', 'businessCommunity'],
    ...
    targets: [
      {
        ...
        blockingText: (s) => (s.secondaryFactions.techSector ?? 40) < 70 ? `Tech sector at ${(s.secondaryFactions.techSector ?? 40).toFixed(0)}. Needs 70.` : null,
      },
      {
        label: 'Ikeja constituency approval at 70+',
        progress: (s) => Math.min(1, (s.constituencyApproval.ikeja ?? 0) / 70),
        blockingText: (s) => (s.constituencyApproval.ikeja ?? 0) < 70 ? `Ikeja approval is ${(s.constituencyApproval.ikeja ?? 0).toFixed(0)}. Yaba/Ikeja corridor needs 70+.` : null,
      },
      {
        label: 'Infrastructure backbone for tech — infra at 65+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 65),
        blockingText: (s) => s.stats.infrastructureScore < 65 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Tech needs reliable power and transport — at least 65.` : null,
      },
    ],
    relevantFactions: ['businessCommunity', 'civilSocietyMedia'],
    recommendedProjectCategories: ['power', 'transport'],
...

  {
    ...
    description: 'Apapa port gridlock is legendary: trucks queue for weeks, goods rot, and the Lagos economy loses billions daily. This path is about fixing the road infrastructure, clearing the contractor backlog, and restoring Apapa as an economic artery rather than a bottleneck.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => s.stats.infrastructureScore < 70 ? `Infrastructure is ${s.stats.infrastructureScore.toFixed(0)}. Port access roads need 70+.` : null,
      },
      {
        label: 'Apapa constituency approval at 65+',
        progress: (s) => Math.min(1, (s.constituencyApproval.apapa ?? 0) / 65),
        blockingText: (s) => (s.constituencyApproval.apapa ?? 0) < 65 ? `Apapa approval at ${(s.constituencyApproval.apapa ?? 0).toFixed(0)}. Residents need to feel the change.` : null,
      },
      {
        label: 'Contractor backlog cleared (≤ 2)',
        progress: (s) => s.stats.contractorBacklog <= 2 ? 1 : Math.max(0, 1 - (s.stats.contractorBacklog - 2) / 10),
        blockingText: (s) => s.stats.contractorBacklog > 2 ? `Backlog at ${s.stats.contractorBacklog.toFixed(1)}. Every stuck contract is a road not built.` : null,
      },
    ],
...

  {
    ...
    description: 'Ikeja is the administrative and commercial spine of Lagos. This path is about building the infrastructure density, transport connectivity, and business environment that makes the corridor thrive from Oshodi to Maryland.',
    flavorClosing: 'Ikeja works — from the government secretariat to the night markets, the corridor delivers.',
    targets: [
      {
        label: 'Infrastructure at 75+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 75),
        blockingText: (s) => s.stats.infrastructureScore < 75 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. The corridor needs 75+.` : null,
      },
      {
        label: 'Ikeja constituency at 70+',
        progress: (s) => Math.min(1, (s.constituencyApproval.ikeja ?? 0) / 70),
        blockingText: (s) => (s.constituencyApproval.ikeja ?? 0) < 70 ? `Ikeja approval at ${(s.constituencyApproval.ikeja ?? 0).toFixed(0)}. Needs 70.` : null,
      },
    ],
...

  {
    ...
    description: 'Lagos was built on water — but the waterfront has been ceded to blight, encroachment, and neglect. This path is about cleaning up the lagoon edge from Lagos Mainland to Lagos Island, building public access, and restoring the economic and recreational potential of the city\'s natural asset.',
    flavorClosing: 'The lagoon is Lagos again — clean, accessible, alive.',
    targets: [
      {
        label: 'Infrastructure at 65+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 65),
        blockingText: (s) => s.stats.infrastructureScore < 65 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Waterfront renewal needs 65+.` : null,
      },
      {
        label: 'Lagos Mainland approval at 60+',
        progress: (s) => Math.min(1, (s.constituencyApproval.lagosMainland ?? 0) / 60),
        blockingText: (s) => (s.constituencyApproval.lagosMainland ?? 0) < 60 ? `Lagos Mainland at ${(s.constituencyApproval.lagosMainland ?? 0).toFixed(0)}.` : null,
      },
      {
        label: 'Lagos Island approval at 65+',
        progress: (s) => Math.min(1, (s.constituencyApproval.lagosIsland ?? 0) / 65),
        blockingText: (s) => (s.constituencyApproval.lagosIsland ?? 0) < 65 ? `Lagos Island at ${(s.constituencyApproval.lagosIsland ?? 0).toFixed(0)}.` : null,
      },
    ],
    relevantFactions: ['civilSocietyMedia', 'businessCommunity'],
...

  {
    ...
    description: 'The yellow danfo buses are Lagos — chaotic, loud, essential, and captured by the NURTW cartels. This path is about formalising transport: weakening the union monopoly, modernising the fleet, and building the BRT infrastructure that gives commuters a real alternative.',
    flavorClosing: 'The danfos still run — but they run on time, on route, and on the state\'s terms.',
    targets: [
      {
        label: 'NURTW influence broken — informalEconomy at 35 or below',
        progress: (s) => s.factions.informalEconomy <= 35 ? 1 : Math.max(0, 1 - (s.factions.informalEconomy - 35) / 40),
        blockingText: (s) => s.factions.informalEconomy > 35 ? `Informal economy is still strong at ${s.factions.informalEconomy.toFixed(0)}. The union needs to be below 35.` : null,
      },
      {
        label: 'Infrastructure for modern transport at 80+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 80),
        blockingText: (s) => s.stats.infrastructureScore < 80 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Modern transport needs 80+.` : null,
      },
      {
        label: 'Commuters feel the difference — trust at 65+',
        progress: (s) => Math.min(1, s.stats.publicTrust / 65),
        blockingText: (s) => s.stats.publicTrust < 65 ? `Only ${s.stats.publicTrust.toFixed(0)}% trust the system. Need 65% for commuters to believe.` : null,
      },
    ],
...

  {
    ...
    description: 'Every Lagosian has a traffic story. This path is about making the big infrastructure bets — rail, BRT, dedicated corridors — that actually move people. It requires infrastructure at scale and projects completed.',
    flavorClosing: 'Lagos moves. Not fast. But it moves — and it never stops.',
    targets: [
      {
        label: 'Infrastructure at 75+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 75),
        blockingText: (s) => s.stats.infrastructureScore < 75 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Mass transit needs 75+.` : null,
      },
      {
        label: 'Public trust in delivery — trust at 60+',
        progress: (s) => Math.min(1, s.stats.publicTrust / 60),
        blockingText: (s) => s.stats.publicTrust < 60 ? `Trust at ${s.stats.publicTrust.toFixed(0)}%. If people do not believe, they will not board.` : null,
      },
    ],
    relevantFactions: ['businessCommunity', 'lgChairmen'],
    recommendedProjectCategories: ['transport'],
...

  {
    ...
    description: 'The Lagos lagoon divides the city as much as it defines it. Ferries, bridges, and water transport are the fastest-growing piece of the transit network. This path is about making the water work for commuters, not just the wealthy with speedboats.',
    flavorClosing: 'The ferry is the fastest commute in Lagos — and anyone can take it.',
    targets: [
      {
        label: 'Infrastructure at 70+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 70),
        blockingText: (s) => s.stats.infrastructureScore < 70 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Water transit needs 70+.` : null,
      },
      {
        ...
        blockingText: (s) => `Mainland (${(s.constituencyApproval.lagosMainland ?? 0).toFixed(0)}) / Island (${(s.constituencyApproval.lagosIsland ?? 0).toFixed(0)}) — both above 65 needed.`,
      },
    ],
    relevantFactions: ['informalEconomy', 'businessCommunity'],
    recommendedProjectCategories: ['transport'],
...

  {
    ...
    description: 'Lagos has over a million children in public schools — and many learn in dilapidated buildings with overcrowded classrooms. This path is about capital efficiency in education spending, renovating distressed schools, and restoring public confidence in the state school system.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => s.stats.capitalEfficiency < 0.8 ? `Capital efficiency at ${(s.stats.capitalEfficiency * 100).toFixed(0)}%. Education spending is leaking.` : null,
      },
      {
        label: 'Public trust in the system at 70+',
        progress: (s) => Math.min(1, s.stats.publicTrust / 70),
        blockingText: (s) => s.stats.publicTrust < 70 ? `Trust at ${s.stats.publicTrust.toFixed(0)}%. Parents need to believe in state schools.` : null,
      },
    ],
    relevantFactions: ['civilSocietyMedia', 'businessCommunity'],
...

  {
    ...
    description: 'From Lassa fever to maternal mortality, Lagos\'s health challenges are as big as the city itself. This path is about strengthening the medical establishment, earning public confidence in health delivery, and building the infrastructure for disease prevention. The medicalAssociation secondary faction tracks the health sector.',
    flavorClosing: 'When a Lagosian falls ill, the state is there — fast, capable, free at the point of care.',
    targets: [
      {
        label: 'Medical association at 65+',
        progress: (s) => Math.min(1, (s.secondaryFactions.medicalAssociation ?? 50) / 65),
        blockingText: (s) => (s.secondaryFactions.medicalAssociation ?? 50) < 65 ? `Medical association at ${(s.secondaryFactions.medicalAssociation ?? 50).toFixed(0)}. Needs 65.` : null,
      },
      {
        label: 'Public trust in health delivery at 65+',
        progress: (s) => Math.min(1, s.stats.publicTrust / 65),
        blockingText: (s) => s.stats.publicTrust < 65 ? `Trust at ${s.stats.publicTrust.toFixed(0)}%. Lagosians do not trust the health system.` : null,
      },
    ],
...

  {
    ...
    description: 'Lagos consumes more food than any city in West Africa — and produces almost none of it. This path is about agricultural logistics: cold chains, farm-to-market canals, livestock corridors, and the market infrastructure that keeps food prices stable and supply reliable.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => (s.stats.foodSecurityIndex ?? 40) < 70 ? `Food security is at ${(s.stats.foodSecurityIndex ?? 40).toFixed(0)}. Below the 70 threshold.` : null,
      },
      {
        label: 'Agrarian sector alignment at 60+',
        progress: (s) => Math.min(1, (s.secondaryFactions.agrarianSector ?? 40) / 60),
        blockingText: (s) => (s.secondaryFactions.agrarianSector ?? 40) < 60 ? `Agrarian sector at ${(s.secondaryFactions.agrarianSector ?? 40).toFixed(0)}. Farmers need to be on side.` : null,
      },
      {
        ...
        blockingText: (s) => `Epe (${(s.constituencyApproval.epe ?? 0).toFixed(0)}) / Badagry (${(s.constituencyApproval.badagry ?? 0).toFixed(0)}) — farming communities below 60.`,
      },
    ],
...

  {
    ...
    description: 'Flooding is Lagos\'s oldest enemy — and climate change is making it worse. This path is about drainage, wetland enforcement, resettlement, and the infrastructure that keeps the city dry when the rains come. The floodResilienceScore stat measures how prepared the city is.',
    ...
    targets: [
      {
        ...
        blockingText: (s) => (s.stats.floodResilienceScore ?? 35) < 65 ? `Flood resilience at ${(s.stats.floodResilienceScore ?? 35).toFixed(0)}. Not ready for the next rainy season.` : null,
      },
      {
        label: 'Infrastructure backbone at 60+',
        progress: (s) => Math.min(1, s.stats.infrastructureScore / 60),
        blockingText: (s) => s.stats.infrastructureScore < 60 ? `Infrastructure at ${s.stats.infrastructureScore.toFixed(0)}. Drainage needs a 60+ base.` : null,
      },
    ],
    relevantFactions: ['civilSocietyMedia', 'businessCommunity'],
