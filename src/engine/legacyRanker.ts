...
function statDeltaToEnglish(delta: Partial<Record<string, number>>): string {
  const parts: string[] = []
  if ((delta.publicTrust ?? 0) !== 0)
    parts.push(`trust ${delta.publicTrust! > 0 ? '+' : ''}${delta.publicTrust!}`)
  if ((delta.cashReserve ?? 0) !== 0)
    parts.push(`cash ₦${delta.cashReserve!.toFixed(1)}bn`)
  if ((delta.infrastructureScore ?? 0) !== 0)
    parts.push(`infrastructure ${delta.infrastructureScore! > 0 ? '+' : ''}${delta.infrastructureScore!}`)
  if ((delta.corruptionPressure ?? 0) !== 0)
    parts.push(`corruption pressure ${delta.corruptionPressure! > 0 ? '+' : ''}${delta.corruptionPressure!}`)
  if ((delta.federalRelationship ?? 0) !== 0)
    parts.push(`federal relations ${delta.federalRelationship! > 0 ? '+' : ''}${delta.federalRelationship!}`)
  if ((delta.youthTension ?? 0) !== 0)
    parts.push(`youth tension ${delta.youthTension! > 0 ? '+' : ''}${delta.youthTension!}`)
  if ((delta.securityIndex ?? 0) !== 0)
    parts.push(`security ${delta.securityIndex! > 0 ? '+' : ''}${delta.securityIndex!}`)
  if ((delta.igr ?? 0) !== 0)
    parts.push(`IGR ${delta.igr! > 0 ? '+' : ''}₦${delta.igr!.toFixed(1)}bn/wk`)
  if ((delta.debtStock ?? 0) !== 0)
    parts.push(`debt ${delta.debtStock! > 0 ? '+' : ''}₦${delta.debtStock!.toFixed(0)}bn`)
  if ((delta.politicalCapital ?? 0) !== 0)
    parts.push(`political capital ${delta.politicalCapital! > 0 ? '+' : ''}${delta.politicalCapital!}`)
...
const FLAG_LABELS: Record<string, string> = {
  ...
  'valedictory-infrastructure': 'Led valedictory address with infrastructure delivery as the defining legacy',
  ...
  'backed-opposition-candidate': 'Secretly backed an opposition candidate who aligned with the reform agenda',
  'vp-deal-accepted': 'Accepted the Vice President\'s mediation: endorsed NEC candidate, dropped VAT case',
  ...
  'deputy-endorsed': 'Endorsed the Deputy Governor\'s gubernatorial campaign in exchange for concessions',
  ...
  'handover-published': 'Released the full administration handover note to the press simultaneously with handover',
  'handover-clean': 'Provided a complete, unredacted official handover note to the incoming administration',
  'financial-disclosure-published': 'Published eight years of full state financial accounts to counter disinformation',
  'sanitation-enforcement-reformed': 'Abolished physical sanitation enforcement after viral footage of an officer\'s death',
  'commissioners-purged': 'Sacked all four commissioners who publicly defected to the rival camp',
  'pivoted-from-state-media': 'Redirected state communications to private media after LTVS journalist mutiny',
  'audit-published-openly': 'Published the PwC administration audit with a public response white paper',
  'legacy-bills-executive-order': 'Implemented blocked legacy legislation via executive order',
  'fire-safety-surge': 'Ordered emergency fire safety inspections citywide after Lagos Island tenement fire',
  'held-high-ground-betrayal': 'Issued no public response when the chosen successor publicly distanced himself',
...
export function buildLegacyPrompt(state: GameState): string {
  ...
  const activeFlags = Object.entries(state.stateFlags)
    ...
    .map(([k]) => FLAG_LABELS[k])
    .filter(Boolean)
  const flagsBlock = activeFlags.length > 0
    ? activeFlags.map((l) => `- ${l}`).join('\n')
